...
The Skill Hub CLI manages the personal distributed skill set in target repositories.
...
`--target standard` is the supported install target. `install` always selects the complete standard skill set: every `kind: "skill"` component in `capabilities/index.json`. The implementation still stores the selected target in existing `agents` lock fields for schema continuity, but the value is the personal default distribution.
