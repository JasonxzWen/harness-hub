...
- Use `insight` for private repository interaction audits from local Codex / Claude Code traces, layered prompt/rule context, automation logs, user-agent task profiles, tool-call decision review, collaboration bottleneck analysis, stale guidance discovery, SOP/script lessons, repeated mistake mining, knowledge-cache candidates, and executable improvement queues. Use `source-post` when external source material becomes a public post, `agent-introspection-debugging` for one failing agent run or tool loop, `harness-quality-check` for advisory harness readiness HTML, and `answer-workflow` for ordinary explanations.
- Use `effective-interact` as the report surface for high-volume `insight` audits when the output needs evidence coverage tables, trace-cluster navigation, bottleneck/recommendation comparison, or HTML handoff validation. `insight` remains the analysis owner.
...
- `insight` loads for private repository interaction insight audits, Codex or Claude Code trace review, agent task profiles, tool-call decision audits, prompt/rule context audits, automation log audits, SOP/script lessons, knowledge-cache candidates, executable improvement queues, and collaboration bottleneck analysis; it does not load for public external-source posts, one failing agent run, advisory harness quality reports, or ordinary summaries.
- `insight` loads for private repository interaction insight audits, Codex or Claude Code trace review, agent task profiles, tool-call decision audits, collaboration bottleneck analysis, session-level insights, and concrete improvement recommendations; it also covers stale guidance, repeated agent mistakes, and rediscovered project knowledge when supported by agent traces or context evidence.
- `insight` loads for session-level insights and concrete improvement recommendations from agent traces, not only private audit wording.
