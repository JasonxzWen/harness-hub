...
| Agentic loop catalog | Standardizes `plan-review`, `test-design`, `implementation-review`, `frontend-acceptance`, `diagnosis-regression`, `pr-closeout`, and `insight-retro`. | Do not make loop types router states or new skill owners. | Install the canonical reference under `skills/workflow-router/references/agentic-loops.md` so target repos do not depend on source `docs/`. |
