...
- `effective-interact` owns communication artifacts such as option comparisons, material change reports, and lightweight export editors.
...
Do not add host-specific install directories to the capability graph. Packaging for a host belongs in that host's manifest layer, such as `.claude-plugin/`.
