...
| Implementation workflow and engineering governance | `sdd-workflow`, `tdd-workflow`, `prototype`, `verification-loop`, `karpathy-guidelines`, lifecycle CLI, agentic loop policy | User-selected: Matt Pocock `prototype`, `tdd`, `scaffold-exercises`; Superpowers `executing-plans`, `finishing-a-development-branch`, `using-git-worktrees`, `subagent-driven-development`, `writing-skills`; `vercel-labs/skills` `find-skills`; `multica-ai/andrej-karpathy-skills` | Strong coverage. Treat Superpowers as source patterns for orchestration policy, not default always-on skills. Karpathy guidelines are a helper baseline, not another lifecycle owner. Agentic loops stay stage mechanics, not a new workflow owner. |
