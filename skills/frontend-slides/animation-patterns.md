...
Every effect needs a quiet equivalent. Keep the final state visible and remove decorative motion when the user requests reduced motion:
...
| Animations not triggering | Check that the presentation controller adds `.visible` to the active fixed-stage slide |
| Stage transition looks wrong | Keep slide layout fixed at 1920×1080; animate content inside the scaled stage, not the stage transform |
| Small-screen issues | Preserve the fixed stage; reduce or disable heavy effects without reflowing slide content |
