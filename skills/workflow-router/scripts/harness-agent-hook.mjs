...
function makeHostOutput(host, hostEvent, gateEvent, gate) {
  ...
  const lifecycleContext = gateEvent === 'user-prompt'
    ? makeWorkflowLifecycleContext(host, gate.route)
    : '';
  if (gate.findings.length === 0 && !lifecycleContext) {
    return null;
  }

  const findingSummary = gate.findings.length > 0 ? summarizeFindings(gate) : '';
  const additionalContext = [lifecycleContext, findingSummary].filter(Boolean).join('\n\n');
  return {
    ...(findingSummary ? { systemMessage: findingSummary } : {}),
    hookSpecificOutput: {
      hookEventName: hostEvent,
      additionalContext,
    },
...
function makeWorkflowLifecycleContext(host, route) {
  if (!isRecord(route)) {
    return '';
  }
  if (!route.owner) {
    return route.state === 'clarify' && typeof route.clarification === 'string'
      ? `Harness Hub Router paused before execution: ${route.clarification}`
      : '';
  }

  const skillRoot = host === 'codex' ? '.agents/skills' : '.claude/skills';
  return [
    'Harness Hub executable Workflow lifecycle:',
    `- Router selected exactly one owner: ${route.owner}.`,
    '- The current main Agent must consume this route; Router output alone is not execution.',
    '- Align and persist the task contract, then invoke the existing Loop runtime. Do not copy Loop internals into the Workflow or create another runner.',
    `- CLI shape: node ${skillRoot}/workflow-router/scripts/loop-runtime.mjs workflow <target> --workflow ${route.owner} --run-id <run-id> --host ${host} --input <task-contract.json> --json`,
    '- The main Agent retains final decisions, conflict handling, authorization checks, and the user-facing report.',
  ].join('\n');
}
...
function summarizeFindings(gate) {
  const findingText = gate.findings
    ...
    .join('; ');
  return findingText || `Project agent gate decision: ${gate.decision}`;
}

function printHelp() {
  console.log(`Usage: node <host-skill-root>/workflow-router/scripts/harness-agent-hook.mjs [target] --host <codex|claude|raw> [--event HookEvent] [--input input.json] [--enforce]

Reads host hook JSON from stdin, runs the local deterministic gate, and emits host-compatible JSON.
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    if (process.env.HARNESS_HUB_CHILD_RUN === '1') {
      process.exitCode = 0;
      process.exit();
    }
