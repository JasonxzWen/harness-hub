...
Craft criteria and starting values for motion review, distilled from Emil Kowalski's design engineering philosophy. Use them to investigate and explain findings, not as deterministic product or merge policy. Verify version-sensitive claims in current official documentation and profile the target product.

## Should it animate? (frequency table)

| Frequency | Decision |
| --- | --- |
| 100+ times/day (keyboard shortcuts, command palette toggle) | Usually omit; retain only with an evidenced purpose and negligible interaction cost |
| Tens of times/day (hover effects, list navigation) | Remove or drastically reduce |
| Occasional (modals, drawers, toasts) | Standard animation |
| Rare / first-time (onboarding, feedback, celebrations) | Can add delight |

**Default to no animation for keyboard-initiated actions** — they repeat hundreds of times daily; animation makes them feel slow and disconnected. (Raycast has no open/close animation — correct for something used hundreds of times a day.)
...
**Avoid `ease-in` on responsive UI by default.** It starts slow, delaying the exact moment the user is watching. `ease-out` at 200ms *feels* faster than `ease-in` at 200ms.

When built-in CSS easings do not fit the interaction, consider stronger custom curves:
...
**Starting point: keep common UI animations under 300ms unless the interaction or product context supports another value.** A 180ms dropdown feels more responsive than a 400ms one. Faster spinners make load feel faster (same actual time). Instant tooltips after the first (skip delay + animation) make a toolbar feel faster.

## Physicality

- **Avoid `scale(0)` for physical entrances.** Start from `scale(0.9–0.97)` + `opacity: 0`. Nothing in the real world appears from nothing.
...
- **Prefer `transform` and `opacity` when they preserve the intended visual result** — eligible animations often avoid layout and may be composited, but paint and compositing depend on the property, element, and browser. Layout properties such as `padding`/`margin`/`height`/`width`/`top`/`left` commonly add layout or paint work; verify the actual path in DevTools.
- **Don't drive child transforms via a CSS variable on the parent** — it recalcs styles for all children. Set `transform` directly on the element.
  ...
  ```
- **Treat Motion transform acceleration as version-sensitive.** Do not assume `x`/`y`/`scale` or a full transform string has a particular execution path; verify current Motion documentation and profile the target:
  ```jsx
  <motion.div animate={{ x: 100 }} />                          // verify on the current Motion version
  <motion.div animate={{ transform: "translateX(100px)" }} />  // compare with profiling evidence
  ```
- **CSS animations may remain smoother than main-thread JavaScript under load** when the browser can run eligible work independently; this is not guaranteed by the API category. Choose CSS for predetermined motion or JavaScript for dynamic, interruptible behavior, then profile the target state.
- **WAAPI** provides native JavaScript timing and playback control without a library. It does not guarantee compositor execution; property choice and browser behavior still require profiling:
  ```js
