...
`init-harness` is the explicit Codex-only dev bootstrap path. It composes the standard skill install with stable harness files such as `AGENTS.md`, `feature_list.json`, `clean-state-checklist.md`, `definition-of-done.md`, and `scripts/harness-validate.mjs`, plus local state templates at `.harness-hub/state/current-task.md`, `.harness-hub/state/decisions.md`, `.harness-hub/state/progress.md`, and `.harness-hub/state/session-handoff.md`. It writes `.harness-hub/.gitignore` so state and generated reports stay local, records ownership in `.harness-hub/lock.json`, blocks dirty git worktrees and existing harness files by default, and leaves low-level `install` skills-only.
