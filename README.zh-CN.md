...
agent 执行规则以 [AGENTS.md](AGENTS.md) 为规范源；[CLAUDE.md](CLAUDE.md) 通过 `@AGENTS.md` 导入给 Claude Code 使用。面向人的开发流程说明放在 [Development Workflow](docs/development-workflow.md)。
...
flowchart TD
  ...
  Start --> SourcePost["发布 source-backed post"]
...
| 我想要... | 从这里开始 | 得到什么 |
|---|---|---|
| 让另一个仓库适合 agent 驱动开发 | `init-harness --target standard` | 标准 skills、规范 `AGENTS.md` 与导入式 `CLAUDE.md`、本地 state 模板、验证脚本、lock ownership。 |
...
| 让已安装 skills 被本地 agent 看见 | `activate-agents --yes` | 把项目内 `skills/<name>` 同步到 `.codex/skills` 和 `.claude/skills`，不做全局安装。 |
| 为本仓库本地 agent 镜像 skills | `bun run sync:agent-skills` | 把源 `skills/<name>` 同步到本地 `.codex/skills` 和 `.claude/skills`。 |
...
`npx ...@latest` 会执行 registry 提供的代码。Agent 只有在当前任务或用户明确授权这类包执行时才应运行它；否则使用已批准的固定版本，或先询问。
...
对无人值守 agent 来说，上面的 `@latest` 示例都属于外部包执行，需要任务授权或已批准的固定版本。
...
`check` 是只读启动检查。它在 `cli` 报告 npm 上的 CLI 包状态，在 `target` 报告目标仓库 lock 托管组件状态，并在 `externalTools` 给出 CodeGraph 和 Headroom 的显式配置/安装建议；更新可用、registry 失败、缺少 lock、缺少项目本地 agent 激活、外部工具建议都只是 advisory，不会安装工具、改写目标仓库或阻塞 agent 启动路径。

`activate-agents` 是显式的项目本地 agent 激活步骤。它把已经安装在目标仓库里的 `skills/<name>` 复制到 `.codex/skills/<name>` 和 `.claude/skills/<name>`，让 Codex 和 Claude Code 都能索引 skill metadata，包括 `package-release-sniffer` 这类 helper 触发。它只写目标仓库本地 host 缓存，用 Harness Hub marker 避免覆盖未标记的本地 skill，不写全局 skill 目录，也不写 `.harness-hub/lock.json`。
...
| 计划与实现 | `grill-me`、`grill-with-docs`、`product-capability`、`tdd-workflow`、`karpathy-guidelines`、`ponytail`、`verification-loop`。 |
...
| `AGENTS.md` / `CLAUDE.md` | 规范 agent 仓库规则和 Claude Code 导入入口。 |
