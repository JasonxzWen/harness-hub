...
Harness Hub 通过仓库 URL 为 Claude Code / Codex 项目提供确定性的 Harness 迁移。

它只有一个面向目标项目的公开命令 `migrate`，并提供两种显式策略：Managed 保留现有的确定性全量迁移，Guided 则把选择性接入以只读方式交给原生 Agent，并要求用户逐层授权。Git 仓库及其 commit 是唯一分发和版本来源；不发布 npm 包，不提供独立更新/删除、兼容入口、通用 Agent Runtime 或第二套 registry。
...
从干净且已有 `HEAD` 的 Harness Hub checkout 显式运行：

```text
node bin/harness-hub.mjs migrate <target> --guided
```

Guided 不会因为目标已有配置或发生 collision 而自动启用，并且与 `--yes`、`--host`、`--primary`、`--force` 互斥。CLI 只验证自身 source commit，并返回现有能力地图与接入范式的路径；不扫描或写入 target，不调用 Host CLI，不创建 manifest，也不进入 rollback。

随后由原生 Agent 检查项目事实，按 `reuse`、`adapt`、`add`、`skip` 给出带证据、影响路径、local/shared 可见性和风险的建议。已有配置只能选择性 patch，不能整文件覆盖。local 新增内容必须保持 untracked，并使用仓库私有 Git exclude；tracked/shared patch 需要先展示精确补丁并单独确认，stage、commit、push、PR、publish 和 merge 仍需再次独立授权。完整的建议表和授权契约见 [BOOTSTRAP-TARGET.md](BOOTSTRAP-TARGET.md)。

## 首次 Managed 迁移或显式切换 Host
...
Codex 项目 Hook 只有在用户已信任目标仓库时运行，Managed 迁移器不会改变 trust。

Harness Hub 自身的 `knowledge/`、来源记录、文档、测试、任务状态和仓库历史永不迁移。

## 原子 Skills 与 Report / Retro

原生主 Agent直接选择有独立领域价值的 Skills。开发原子能力包括 `grill-me`、`grill-with-docs`、`product-capability`、`to-tickets`、`tdd`、`codebase-design`、`code-review`、`product-ui-review` 和 `verification`；产物原子能力包括 `frontend-slides`、`codebase-to-course` 和 `beautiful-feishu-whiteboard`；其他能力包括 Ponytail、effective-interact、quick-learn、source-post、Agent Reach、Decision UI、安全审查和 Agent interaction audit。
...
- HTML 演示文稿路由到 `frontend-slides`，基于源码的交互课程路由到 `codebase-to-course`，可编辑的飞书/Lark 可视化讲解路由到 `beautiful-feishu-whiteboard`。这些能力不安装依赖、不配置认证；导出、部署、发布和线上白板写入仍遵守原有授权边界。
...
## Managed OKF / LLM-Wiki

首次 Managed 迁移只有在目标既没有旧 manifest、也没有 `knowledge/` 时，才调用 primary CLI 一次，基于真实目标源码生成来源可追溯的 Google OKF v0.1 Wiki。
...
Managed 迁移器保留确定性 ownership、stale cleanup、Git control snapshot、受保护路径验证和 rollback。若无关 ignored 内容或源仓库在迁移期间发生变化导致无法精确恢复，迁移器保留不属于它的修改并返回 `rolledBack: false`，不得伪造回滚成功。
