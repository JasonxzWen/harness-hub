...
agent 执行规则放在同步维护的 [AGENTS.md](AGENTS.md) 和 [CLAUDE.md](CLAUDE.md)。面向人的开发流程说明放在 [Development Workflow](docs/development-workflow.md)。
...
| 我想要... | 从这里开始 | 得到什么 |
|---|---|---|
| 让另一个仓库适合 agent 驱动开发 | `init-harness --target standard` | 标准 skills、同步根级 harness 文件、本地 state 模板、验证脚本、lock ownership。 |
...
| `AGENTS.md` / `CLAUDE.md` | 同步维护的 agent 仓库规则和执行工作流。 |
