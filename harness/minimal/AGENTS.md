...
- During finish closeout, run or explicitly skip `agent-interaction-audit` to review tool-calling quality, repeated low-value lookup loops, misleading evidence, code/docs conflicts, AI infrastructure lessons, and whether the lesson should become a project rule, validation case, documentation, automation check, or follow-up task.
...
Before ending a session, update `.harness-hub/state/session-handoff.md` with the current status, changed files, validation evidence, stale-read gate result, agentic loop records, finish closeout findings, agent interaction audit recommendations, PR status and PR handoff details when a PR was created or updated, blockers, and the next concrete action. Update `.harness-hub/state/decisions.md` when assumptions, acceptance criteria, allowed paths, validation, user-visible behavior, or risk changed.
