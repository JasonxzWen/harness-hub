...
- Before final handoff for any mutation, run the required closeout loop based on changed paths: use a subagent or independent review pass when the required evidence level calls for isolation and the scope is safe; otherwise record a fallback reason and deterministic substitute. Small source/test changes still need implementation or test review evidence. Expose findings instead of burying them.
