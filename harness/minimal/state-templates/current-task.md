...
For mutation work, derive required loops from dirty paths when the CLI runtime is available with `harness-hub loop required . --json`, or from checkpoint commits with `harness-hub loop required . --base <ref> --head <ref> --json`; then verify run and integration evidence with `harness-hub loop verify . --input <verify.json> --json` before handoff. Small changes can use lower evidence, but they still need review or explicit unresolved findings.
