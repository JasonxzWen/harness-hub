...
### Requirement: Source research produces project-aware source posts
The system SHALL transform user-provided source material into a UTF-8 source post that extracts viewpoints, integrates them, maps them to Harness Hub, and records iteration impact.

#### Scenario: Generate source post draft
- **WHEN** the user provides one or more source URLs or source texts
- **THEN** the system records source metadata
- **AND** the generated post identifies source claims, extracted viewpoints, integrated interpretation, Harness Hub impact, iteration record, action boundary, and source references
...
The system SHALL use `effective-interact` as the initial generation layer for source post drafts.

#### Scenario: Generate draft with effective-interact
- **WHEN** a source post draft is generated
...
The system SHALL store public post source and generated site files in a versioned site surface that is separate from ignored local report artifacts.

#### Scenario: Write post source
- **WHEN** a source post is accepted for publication
...
The system SHALL publish source posts to GitHub Pages only through an explicit command or automation path with preflight checks.
...
### Requirement: Source posts create iteration records
